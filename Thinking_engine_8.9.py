# Thinking_Engine_8.9.py
# ------------------------------------------------------------
# Version: 8.9 | Goal Linking + Contextual Planning
# Keeps: 8.8 features (MD/CSV/JSON auto-exports, decay, merge, drift)
# ------------------------------------------------------------
import json, os, argparse, time, csv, re, uuid
from datetime import datetime, timezone
from typing import Any, Dict, List, Tuple, Optional

VERSION = "8.9"
SCHEMA_VERSION = 12   # safe migration
...
class Memory:
    ...
    def load(self):
        ...
        base = {
            ...
            "reflections": [],
            "facts": [],
            "goals": [],                # NEW: list of {id,title,created,last_update}
            "open_tasks": [],
            "completed_tasks": [],
            "task_scores": {},
            "task_goal": {},            # NEW: task_text -> goal_id
            "errors": [],
            "insights": [],
            "confidence_scores": {},
            "insight_weights": {},
    ...
    def add_task(self, t: str, s: float, goal_id: Optional[str] = None):
        ...
        if key in norm_map:
            ...
            # keep a goal link if we don't have one; otherwise preserve existing
    ...
    def maintenance(self, decay: float = 0.98, floor: float = 0.10):
        ...
        # rebuild task_goal with survivors
        self.data["task_goal"] = {}
    ...
    def calc_goal_metrics(self) -> List[Dict[str, Any]]:
        # recompute counts based on current task_goal map
        ...
        # sort by (open desc, progress asc)
...
class Engine:
    ...
    def derive_goal_title(self, insight: str, emotion: str) -> str:
        """
        Produces a human-friendly goal title from an insight.
        Example:
          "Insight: derived from motivated state — Plan: Execute a high-confidence action now."
          -> "Execute high-confidence actions"
        """
        if not insight:
            return f"{emotion.title()} → Clarify next objective"
        # extract after '—' if present, clean verbs
        part = insight
        if "—" in insight:
            part = insight.split("—", 1)[-1]
        part = part.strip().rstrip(".")
        part = re.sub(r"^(Plan:|Define:)\s*", "", part, flags=re.I)
        # soften imperative to noun phrase
    ...
    def reflect_once(self) -> Dict[str, Any]:
        ...
        # Every 3 reflections → insight + tasks (now linked to a goal)
        new_tasks = []
        if len(m.data["reflections"]) % 3 == 0:
            ...
            # derive or reuse a goal for this insight cluster
            goal_title = self.derive_goal_title(ins, emotion)
...
def banner(mem: Memory, r: Dict[str, Any]) -> str:
    ...
    # show focused goal (if the top task is linked)
...
def export_csv(mem: Memory) -> str:
    ...
    goals = mem.calc_goal_metrics()
    # collapse top 3 goals to a single field
...
def main():
    parser = argparse.ArgumentParser(description="Thinking Engine 8.9")
