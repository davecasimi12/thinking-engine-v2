# ------------------------------------------------------------
# Thinking_Engine_18.2.py
# Phase 18.2 — Unified Console Mode
# - Keeps Phase 18.0: Owner-Lock, offline guard, hashed exports/feeds, audit log,
#   greeting, local read-only HTTP mirror, family feeds (Nicole/Sam/Jon)
# - Adds non-blocking console input thread + command router
# ------------------------------------------------------------
...
# Set False if you want the local HTTP endpoints; keep True for strict offline (or run with --no-http)
OFFLINE_MODE = False

HTTP_HOST = "127.0.0.1"
HTTP_PORT = 5050
SLEEP_SEC_BASE = 5.0
GREETING_INTERVAL_CYCLES = 6   # friendly heartbeat every N cycles
...
def write_greeting(status: str, message: str = None) -> None:
    if message is None:
        message = "Hey YZ 👋 I’m online — ready when you are."  # casual style
...
def _input_thread(stop_event: threading.Event, inbox: "queue.Queue[str]"):
    # Separate thread: blocks on input(), pushes lines into a queue
    while not stop_event.is_set():
        try:
            line = input().strip()
        except EOFError:
...
def _handle_command(cmd: str, sleep_sec: float) -> Tuple[bool, float]:
    """Return (should_stop, new_sleep_sec)."""
    lower = cmd.lower().strip()
    _append_session({"ts": iso_now(), "kind": "interaction", "text": cmd})

    if lower in {"hi", "hello", "hey"}:
        _respond("Nova: Hey YZ 👋 I’m here and thinking.")
    ...
    if lower.startswith("sleep "):
        try:
            ...
            _respond(f"Nova: Cycle sleep set to {sec:.1f}s.")
            return (False, sec)
        except Exception:
            _respond("Nova: Give me a number like `sleep 8`.")
            return (False, sleep_sec)

    if lower in {"status","/status"}:
        bun = _read_json(os.path.join(EXPORT_DIR, "sync_bundle.json"), {})
        loop = bun.get("loop", {})
        _respond(f"Nova: status → resilience={loop.get('resilience')} last_ms={loop.get('last_cycle_ms')} recalled={loop.get('recalled')}")
        return (False, sleep_sec)

    if lower in {"recall"}:
        items = recall_core_memories(limit=5)
        _respond(f"Nova: recalled {len(items)} items.")
        return (False, sleep_sec)

    if lower in {"heal"}:
        stat, res = heal_memory()
        _respond(f"Nova: heal run → {stat}")
        return (False, sleep_sec)

    if lower in {"stop","exit","quit"}:
        _respond("Nova: Stopping now. 👋")
        return (True, sleep_sec)

    _respond("Nova: Commands → hello | status | recall | heal | sleep <sec> | stop")
    return (False, sleep_sec)
...
def self_mechanism(stop_event: threading.Event, inbox: "queue.Queue[str]"):
    ...
    while not stop_event.is_set():
        t0 = time.time()
        try:
            # Process any queued console commands first (non-blocking)
            while True:
                try:
                    cmd = inbox.get_nowait()
                except queue.Empty:
                    break
                should_stop, sleep_sec = _handle_command(cmd, sleep_sec)
                if should_stop:
                    stop_event.set()
                    break
        ...
        # Sleep, but let us break out quickly if stop_event is set
...
def main():
    parser = argparse.ArgumentParser(description="Thinking Engine 18.2 — Unified Console Mode")
