# Thinking_Engine_8.7.py
# ------------------------------------------------------------
# Version: 8.7 | Smart Auto-Export (MD + CSV + JSON) + Status
# ------------------------------------------------------------
...
VERSION = "8.7"
SCHEMA_VERSION = 10   # safe carry-forward; migrates from earlier versions
...
class Memory:
    ...
    def load(self):
        ...
        # quarantine unknown keys from older files
...
def export_markdown(mem: Memory) -> str:
    ...
    if insights:
        # try to show the real weight key if possible
...
def export_csv(mem: Memory) -> str:
    ...
    row = {
        ...
        "next_hint": "n/a",  # left placeholder; next hint changes per cycle
...
def main():
    parser = argparse.ArgumentParser(description="Thinking Engine 8.7")
    ...
    if args.status:
        ...
        # Also print the JSON content for a quick CLI glance
    ...
    # Final banner + SMART AUTO-EXPORT (MD + CSV + JSON)
