# ------------------------------------------------------------
# Thinking_Engine_15.0.py
# Phase 15 — Output Channels Layer (Local-only HTTP mirror)
# - Owner-Lock (owner == "YZ")
# - Offline-only (no external modules; local 127.0.0.1 binding)
# - Read-only HTTP endpoints that mirror data/exports/*
# Carries forward: Self loop, Healing 2.0-lite, Exports/Imports, Autonomy-lite
# ------------------------------------------------------------

import os, json, time, uuid, argparse, sys, re, threading
...
def evolve_engine(reflection: str, heal_status: str, recalled: List[Dict[str, Any]]) -> str:
    ...
    # emotion summary (avg valence from reflections)
    ...
    _write_json(EMOTION_SUMMARY_PATH, emo)
    ...
    _write_json(os.path.join(REPORT_DIR, f"cycle_{int(time.time())}.json"), rep)
...
def import_commands_if_any() -> Dict[str, Any]:
    ...
    if cmds.get("clear_commands"):
        _write_json(path, {}); effects["cleared"] = True
    return effects

# ---------- Local HTTP server (read-only mirror) ----------
class _LocalHandler(BaseHTTPRequestHandler):
    # Block any non-local Host attempt explicitly
    ...
    def do_GET(self):
        if not self._only_local():
            self._send_json(403, {"error": "local-only"})
            return

        path = self.path.split("?")[0]
        if path == "/status":
            hb = _read_json(os.path.join(EXPORT_DIR, "heartbeat.json"), {})
            self._send_json(200, {"version": VERSION, "heartbeat": hb})
        elif path == "/reflection":
            ref = _read_json(os.path.join(EXPORT_DIR, "reflection.json"), {})
            self._send_json(200, {"reflection": ref})
        elif path == "/metrics":
            met = _read_json(os.path.join(EXPORT_DIR, "metrics.json"), {})
...
def _start_http_server(stop_event: threading.Event):
    ...
    server.timeout = 0.5  # check stop_event periodically
    _safe_print(f"[HTTP] Local mirror on http://{HTTP_HOST}:{HTTP_PORT} (read-only)")
...
def main():
    parser = argparse.ArgumentParser(description="Thinking Engine 15.0")
    ...
    if OFFLINE_MODE:
        ...
        # allow stdlib http.server only (used above)
