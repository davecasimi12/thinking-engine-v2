# Thinking_Engine_8.6.py
# ------------------------------------------------------------
# Version: 8.6 | Markdown Session Exporter + Autoloop + Drift
# ------------------------------------------------------------
...
VERSION = "8.6"
SCHEMA_VERSION = 9  # unchanged from 8.5 (safe carryover)
...
class Engine:
    ...
    # Priority chaining from history
...
def export_markdown(mem: Memory) -> str:
    ...
    if insights:
        for i, ins in enumerate(insights, 1): lines.append(f"{i}. {ins}  *(w={weights.get(str(len(mem.data['insights'])-len(insights)+i), '—')})*")
    else:
        lines.append("- n/a")
    lines.append("")
    ...
    filename = os.path.join(REPORT_DIR, f"Session_{_short_ts()}.md")
...
# ---------- Main ----------
def main():
    parser = argparse.ArgumentParser(description="Thinking Engine 8.6")
    ...
    parser.add_argument("--export", action="store_true", help="Export a Markdown report and exit.")
    ...
    if args.do:
        ...
        # export quick report after action
        path = export_markdown(mem)
